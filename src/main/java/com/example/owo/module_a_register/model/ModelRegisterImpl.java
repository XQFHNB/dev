...
import com.example.owo.utils.util_http.URL;
...
public class ModelRegisterImpl implements ModelRegister {
    ...
    @Override
    public void register(final HashMap<String, String> map, final OnLoadListener listener) {
     ...
     */
    @Override
    public void registerPoseCamera(final HashMap<String, String> map, OnLoadListener listener) {
        Timer timer = new Timer();
        TimerTask task = new TimerTask() {
            @Override
            public void run() {
        ...
        TimerTask task = new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    UtilLog.d("test", "注册pose相机返回的数据 " + jsonResultData);
                    ...
                    HttpHelper.postStatus(Common.userSP.getInt("ID", 0) + "", Common.COMPLETE + "");
